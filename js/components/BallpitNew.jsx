...
class Z extends d {
    constructor(e, t = {}) {
        ...
        // FIX: Safeguard against crash if envMapRotation is undefined
        if (r.envMapRotation) {
    ...
    update(e) {
        if (this.physics) {
            ...
            for (let idx = 0; idx < this.count; idx++) {
                ...
                if (idx === 0 && (this.config.followCursor === false || this.config.controlSphere0)) {
                    U.scale.setScalar(0);
                } else {
...
const Ballpit = ({ className = '', followCursor = true, ...props }) => {
    ...
    useEffect(() => {
        ...
        try {
            console.log('Mounting Full Ballpit Logic');
            spheresInstanceRef.current = createBallpit(canvas, { followCursor, ...props });
        } catch (err) {
            console.error('Error mounting Ballpit:', err);
        }
